/*
  ...
  1. **profiles**
     ...
     - `avatar_url` (text)
     - `bvn` (text) - Bank Verification Number for vendors
     - `account_number` (text) - Bank account for vendors
     - `account_name` (text) - Account holder name
     - `bank_name` (text) - Name of bank
     - `available_balance` (numeric) - Vendor earnings balance
  ...
  9. **vendor_upgrade_requests**
     - Stores user requests to upgrade from customer to vendor
     - `id` (uuid, primary key)
  ...
  10. **cashout_requests**
      - Vendor cashout/withdrawal requests
      - `id` (uuid, primary key)
  ...
  - Customers: Can view own data, create orders, write reviews, request vendor upgrade
  - Vendors: Can manage own stores and products, view own orders, request cashouts
  - Admins: Full access to all data, can approve vendor upgrades and cashouts
...
-- Create vendor_upgrade_requests table
CREATE TABLE IF NOT EXISTS vendor_upgrade_requests (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
ALTER TABLE vendor_upgrade_requests ENABLE ROW LEVEL SECURITY;

-- Vendor upgrade request policies
CREATE POLICY "Users can create own upgrade request"
  ON vendor_upgrade_requests FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can view own upgrade requests"
  ON vendor_upgrade_requests FOR SELECT
  TO authenticated
...
CREATE POLICY "Admins can update upgrade requests"
  ON vendor_upgrade_requests FOR UPDATE
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM profiles
...
-- Cashout request policies
CREATE POLICY "Vendors can create own cashout requests"
...
CREATE POLICY "Vendors can view own cashout requests"
...
CREATE INDEX IF NOT EXISTS idx_upgrade_requests_user ON vendor_upgrade_requests(user_id);
CREATE INDEX IF NOT EXISTS idx_upgrade_requests_status ON vendor_upgrade_requests(status);
CREATE INDEX IF NOT EXISTS idx_cashout_requests_vendor ON cashout_requests(vendor_id);
...
DROP TRIGGER IF EXISTS update_vendor_upgrade_requests_updated_at ON vendor_upgrade_requests;
CREATE TRIGGER update_vendor_upgrade_requests_updated_at
  BEFORE UPDATE ON vendor_upgrade_requests
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_cashout_requests_updated_at ON cashout_requests;
